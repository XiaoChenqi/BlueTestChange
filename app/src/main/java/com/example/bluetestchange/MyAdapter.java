...
public class MyAdapter extends BaseAdapter {
    ...
    private List<MyBluetoothDevice> mBluelist;
    private LayoutInflater layoutInflater;

    public MyAdapter(Context context, List<MyBluetoothDevice> list) {
        this.mBluelist = list;
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        BluetoothDevice blueDevice = mBluelist.get(position).getBluetoothDevice();

        //信号强度。
        int rssiStr = mBluelist.get(position).getRssi();
        //设备名称
        ...
        if (deviceState == 10) {
            viewHolder.deviceState.setText("未匹配"+"--信号强度："+rssiStr);
        } else if (deviceState == 11) {
            viewHolder.deviceState.setText("绑定中"+"--信号强度："+rssiStr);
        } else if (deviceState == 12) {
            viewHolder.deviceState.setText("已绑定"+"--信号强度："+rssiStr);
        }
